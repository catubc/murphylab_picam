...
static PyObject* py_HX711_new (PyObject *self, PyObject *args) {
	...
	float sp;
	int initGPIO;
	if (!PyArg_ParseTuple(args,"iifi", &dp, &cp, &sp, &initGPIO)) {
		return NULL;
	}
	return PyCapsule_New(static_cast <void *>(new HX711(dp, cp, sp, static_cast <bool>(initGPIO))), "HX711", py_HX711_del);
}
...
static struct PyModuleDef HX711module = {
  ...
  "Controls an HX711 Load Cell Amplifier using wiringPi",  /* Doc string (may be NULL) */
